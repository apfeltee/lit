...
void internal_free_statements(LitState* state, LitStatements* statements)
{
    if(statements == NULL)
    {
        return;
...
#define ALLOCATE_EXPRESSION(state, type, object_type) (type*)allocate_expression(state, line, sizeof(type), object_type)
...
#define ALLOCATE_STATEMENT(state, type, object_type) (type*)allocate_statement(state, line, sizeof(type), object_type)
