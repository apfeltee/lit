...
static void consume(LitParser* parser, LitTokenType type, const char* error)
{
    ...
    string_error(parser, &parser->current,
                 lit_format_error(parser->state, parser->current.line, ERROR_EXPECTATION_UNMET, error,
                                  line ? 8 : parser->previous.length, line ? "new line" : parser->previous.start)
                 ->chars);
}
...
static void parse_parameters(LitParser* parser, LitParameters* parameters)
{
    ...
    while(!check(parser, TOKEN_RIGHT_PAREN))
    {
        ...
        size_t arg_length = parser->previous.length;
...
static LitExpression* parse_grouping_or_lambda(LitParser* parser, bool can_assign)
{
    ...
    if(prs_match(parser, TOKEN_IDENTIFIER) || prs_match(parser, TOKEN_DOT_DOT_DOT))
    {
        ...
        size_t first_arg_length = parser->previous.length;
        ...
        if(prs_match(parser, TOKEN_COMMA) || (prs_match(parser, TOKEN_RIGHT_PAREN) && prs_match(parser, TOKEN_ARROW)))
        {
            ...
            bool had_default = prs_match(parser, TOKEN_EQUAL);
...
static LitExpression* parse_compound(LitParser* parser, LitExpression* prev, bool can_assign)
{
    ...
    = lit_create_binary_expression(parser->state, line, prev, expression, convert_compound_operator(op));
...
static LitExpression* parse_object(LitParser* parser, bool can_assign)
{
    ...
    while(!check(parser, TOKEN_RIGHT_BRACE))
    {
        ...
        lit_values_write(parser->state, &object->keys,
                         OBJECT_VALUE(lit_copy_string(parser->state, parser->previous.start, parser->previous.length)));
...
static LitExpression* parse_dot(LitParser* parser, LitExpression* previous, bool can_assign)
{
    ...
    {
        LitExpression* expression
        = (LitExpression*)lit_create_get_expression(parser->state, line, previous, name, length, false, ignored);
...
static LitExpression* parse_super(LitParser* parser, bool can_assign)
{
    ...
    if(!(prs_match(parser, TOKEN_DOT) || prs_match(parser, TOKEN_SMALL_ARROW)))
    {
        LitExpression* expression = (LitExpression*)lit_create_super_expression(
...
static LitExpression* parse_reference(LitParser* parser, bool can_assign)
{
    ...
    = lit_create_reference_expression(parser->state, line, parse_precedence(parser, PREC_CALL, false));

    if(prs_match(parser, TOKEN_EQUAL))
    {
        return (LitExpression*)lit_create_assign_expression(parser->state, line, (LitExpression*)expression,
                                                            parse_expression(parser));
    }
...
{
    size_t line = parser->previous.line;
    bool invert = prs_match(parser, TOKEN_BANG);

    bool had_paren = prs_match(parser, TOKEN_LEFT_PAREN);
...
static LitStatement* parse_for(LitParser* parser)
{
    ...
    bool c_style = !prs_match(parser, TOKEN_IN);
...
{
    bool isexport = parser->previous.type == TOKEN_EXPORT;
    ...
    size_t function_length = parser->previous.length;
    ...
    if(prs_match(parser, TOKEN_DOT))
    {
        ...
        LitSetExpression* to = lit_create_set_expression(
...
= { TOKEN_PLUS,         TOKEN_MINUS, TOKEN_STAR,       TOKEN_PERCENT, TOKEN_SLASH,         TOKEN_SHARP,

    TOKEN_BANG,         TOKEN_LESS,  TOKEN_LESS_EQUAL, TOKEN_GREATER, TOKEN_GREATER_EQUAL, TOKEN_EQUAL_EQUAL,
