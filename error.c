...
LitString* lit_vformat_error(LitState* state, size_t line, LitError error, va_list args)
{
    ...
    if(line != 0)
    {
        rt = AS_STRING(lit_string_format(state, "[err # line #]: $", (double)error_id, (double)line, (const char*)buffer));
    }
    else
    {
        rt = AS_STRING(lit_string_format(state, "[err #]: $", (double)error_id, (const char*)buffer));
    }
    free(buffer);
    return rt;
}
