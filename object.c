...
bool lit_is_callable_function(LitValue value)
{
    if(IS_OBJECT(value))
    {
        ...
        return type == OBJECT_CLOSURE || type == OBJECT_FUNCTION || type == OBJECT_NATIVE_FUNCTION || type == OBJECT_NATIVE_PRIMITIVE
               || type == OBJECT_NATIVE_METHOD || type == OBJECT_PRIMITIVE_METHOD || type == OBJECT_BOUND_METHOD;
    }
...
LitFiber* lit_create_fiber(LitState* state, LitModule* module, LitFunction* function)
{
    ...
    size_t stack_capacity = function == NULL ? 1 : (size_t)lit_closest_power_of_two(function->max_slots + 1);
...
void lit_ensure_fiber_stack(LitState* state, LitFiber* fiber, size_t needed)
{
    ...
    size_t capacity = (size_t)lit_closest_power_of_two((int)needed);
    LitValue* old_stack = fiber->stack;

    fiber->stack
    = (LitValue*)lit_reallocate(state, fiber->stack, sizeof(LitValue) * fiber->stack_capacity, sizeof(LitValue) * capacity);
    fiber->stack_capacity = capacity;

    if(fiber->stack != old_stack)
    {
        for(size_t i = 0; i < fiber->frame_capacity; i++)
        {
